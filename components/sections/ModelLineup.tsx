...
export default function ModelLineup({ products: suppliedProducts }: { products?: Product[] }) {
  const products = suppliedProducts ?? getProducts();
          ...
          <Card key={p.slug} className="hover:shadow-xl transition-shadow">
            <Link href={`/product/${p.slug}`}>
              ...
              <div className="p-4">
                ...
                <p className="text-sm font-medium uppercase tracking-[0.12em] text-primary">{p.bikeType ?? p.category.replace("-", " ")}</p>
                <p className="text-sm text-onSurface mb-2 line-clamp-2">
                  {p.shortDescription}
                </p>
                <p className="text-xs leading-5 text-gray-400">{p.specs.motor ?? Object.values(p.specs.categorySpecific ?? {})[0]}</p>
